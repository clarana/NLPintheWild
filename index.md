...
<table class="schedule">
  ...
  {% for s in site.data.schedule %}
    ...
    <tr{% if row_class != "" %} class="{{ row_class | strip }}"{% endif %}>
      ...
      <td>{% if s.no_class %}<em>{{ s.lecture }}</em>{% else %}{% if s.lecture_no %}<span class="lec-no">L{{ s.lecture_no }}</span> {% endif %}{% if s.slides %}<a href="{{ s.slides | relative_url }}">{{ s.lecture }}</a>{% else %}{{ s.lecture }}{% endif %}{% if s.speaker %} ({% if s.speaker.url %}<a href="{{ s.speaker.url }}">{{ s.speaker.name }}</a>{% else %}{{ s.speaker.name }}{% endif %}){% endif %}{% endif %}</td>
